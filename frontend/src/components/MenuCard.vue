  ...
  <div class="card overflow-hidden hover:shadow-md transition-shadow group flex flex-col h-full">
    ...
    <div class="p-4 flex flex-col flex-1">
      <div class="flex items-start justify-between gap-2 mb-1">
        <h3 class="font-semibold text-gray-900 dark:text-gray-100 leading-tight">{{ item.name }}</h3>
      ...
      <div class="relative mb-3 flex-1">
        <p v-if="item.description" class="text-gray-500 dark:text-gray-400 text-sm line-clamp-2 peer">{{ item.description }}</p>
      ...
      <div v-else class="flex items-center justify-between bg-orange-50 dark:bg-orange-900/20 rounded-lg p-2">
        <button
          @click="cart.updateQuantity(item.id, quantityInCart - 1)"
          class="w-8 h-8 rounded-full bg-white dark:bg-gray-700 border border-orange-300 dark:border-orange-700 hover:bg-orange-100 dark:hover:bg-orange-900/30 font-bold text-orange-600"
        >−</button>
        <span class="font-semibold text-orange-700 dark:text-orange-400">{{ quantityInCart }} in cart</span>
        <button
          @click="cart.addItem(item)"
