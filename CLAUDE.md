...
- **`views/PaymentView.vue`** — Three payment tabs: QR Code (generated with `qrcode` library from order ref), Card (Luhn validation), Cash at Cashier. Retrieves `paymentToken` from `ordersStore` and sends it in the confirm request body. After confirmation, shows a `useDialog().showAlert()` success message and then `await router.push('/my-orders')`. The navigation is awaited deliberately — without it, the `finally` block would flip `paying` back off before navigation settled and a silent router rejection would leave the user frozen on the payment page.
- **`views/MyOrdersView.vue`** — Shows order history scoped to `tableStore.tableNumber`. On load, checks if the table session is still OPEN (via `tableSessionApi.getSession`). If session is ended, clears orders for that table and resets the table store. Shows three states: no table selected / no orders placed / order list. Loading state renders a fixed 2-card skeleton — **do not** reference `orderNumbers.length` or any other ref that isn't declared in `<script setup>`, as a template render error there makes the entire view blank (regression fixed in commit 7738b94).
